...
train_losses, val_losses = trainer.update(train_dataset)
...
with torch.no_grad():
    ...
    y_pred = y_pred[..., 0] # ELLIPSIS 객체
    y_pred_logvar = y_pred_logvar[..., 0]

y_var_epi = y_pred.var(dim=0).cpu().numpy() # torch.var : calculate variance
y_var = y_pred_logvar.exp()
y_pred = y_pred.mean(dim=0).cpu().numpy()
y_var_ale = y_var.mean(dim=0).cpu().numpy()

# y_var_epi : 예측된 결과값 기준의 variance
 # y_var_ale : 예측한 log variance의 exp의 평균값
